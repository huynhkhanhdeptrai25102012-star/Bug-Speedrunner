(() => {
  ...
  function openRankReview(rankId) {
    ...
    root.innerHTML = `<div class="rank-review-backdrop"><div class="rank-review-modal ${rank.id}"><div class="profile-topline"><span class="eyebrow">RANK REVIEW</span><button class="close-btn" id="rankReviewClose">✕</button></div><div class="rank-review-stage ${rank.id}"><div class="review-avatar ${rank.id}">${avatar}<span class="review-rank-aura">${rank.id === 'orbit' ? '<i class="orbit-body"></i><i class="orbit-ring ring-a"></i><i class="orbit-ring ring-b"></i><i class="orbit-ring ring-c"></i>' : ''}</span></div><div class="review-player"><div class="review-player-name rank-text-${rank.id}">${escapeHtml(displayName)}</div><div class="review-player-rank">${rankIconSvg(rank, 'mini')} ${escapeHtml(rank.name)}</div></div></div>${rank.id==='cane'?`<div class="cane-blackhole-banner"><img src="${V51.ownerGif}" alt="Owner black hole effect"><span>OWNER SINGULARITY</span></div>`:''}<div class="rank-review-grid"><div><small>ICON</small><div class="review-icon ${rank.id}">${rankIconSvg(rank, 'normal')}</div></div><div><small>NAME COLOR</small><div class="review-name-sample rank-text-${rank.id}">${escapeHtml(displayName)}</div></div><div><small>EFFECT</small><div class="review-effect-badge ${rank.id}">${escapeHtml(rank.effect || 'none')}</div></div><div><small>THRESHOLD</small><div class="review-threshold">${rank.threshold} clears</div></div></div><div class="rank-review-footer"><span>${escapeHtml(rank.description)}</span>${audioForRank(rank.id) ? `<span class="audio-note">♫ ${rank.id === 'emperor' ? 'sovereign.mp3' : rank.id === 'orbit' ? 'orbitsong.mp3' : rank.id === 'admin-special' ? 'adminsong.mp3' : 'canesong.mp3'}</span>` : ''}</div></div></div>`;
  ...
  const ACHIEVEMENT_DEFS=[
    ...
    {id:'admin-exclusive',icon:'🔴',name:'Addmin',desc:'Được trao tự động khi tài khoản có role Admin.',target:1,metric:'roleAdmin',title:'Artmean',rarity:'special',specialRole:'admin'},
    {id:'owner-exclusive',icon:'⚫',name:'TheBoss',desc:'Được trao tự động khi tài khoản có role Owner.',target:1,metric:'roleOwner',title:'OhMyBoss!?',rarity:'special',specialRole:'owner'}
  ];
  const DEFAULT_EVENTS=[{id:'ishow-v1',name:'IShow⚡',icon:'⚡',status:'LIVE',version:1,description:'10 challenge liên tục, càng về sau càng khó. Dưới 5 phút để nhận title.',challengeCount:10,timeLimitMs:300000,titleReward:'IShow⚡',achievementReward:'ishow',medalThresholds:{bronze:420000,silver:360000,gold:300000,lightning:270000},difficulty:'rising',challengeLanguage:'any',boss:true}];
...
TIME ${formatMs(total)}`);state.eventSession=null;state.finishing=false;setReadOnly(true);updateActionButtons()}
  ...
  function adminCreateEvent(){const name=prompt('Tên Event mới:','NEW EVENT');if(!name)return;const e={id:'event-'+Date.now().toString(36),name:name.trim().slice(0,60),icon:'🎪',status:'DRAFT',version:1,description:'Custom event',challengeCount:10,timeLimitMs:300000,titleReward:name.trim().slice(0,30),achievementReward:null,medalThresholds:{bronze:420000,silver:360000,gold:300000,lightning:270000},difficulty:'rising',challengeLanguage:'any',boss:true};const es=eventStore();es.push(e);saveEventStore(es);saveCloudEvent(e);renderAdmin();showToast('Draft Event đã tạo.','success')}
  function adminEditEvent(btn){const id=btn.closest('.admin-event-card').querySelector('.admin-event-meta').dataset.event,es=eventStore(),e=es.find(x=>x.id===id);if(!e)return;const name=prompt('Tên Event:',e.name);if(name===null)return;const desc=prompt('Mô tả:',e.description||'');if(desc===null)return;const count=Number(prompt('Số challenge:',e.challengeCount));if(!Number.isFinite(count)||count<1||count>30)return showToast('Số challenge phải từ 1 đến 30.','error');const limit=Number(prompt('Tổng thời gian (giây):',Math.round((e.timeLimitMs||300000)/1000)));if(!Number.isFinite(limit)||limit<30||limit>3600)return showToast('Thời gian phải từ 30 đến 3600 giây.','error');const reward=prompt('Title reward:',e.titleReward||'');if(reward===null)return;const lang=prompt('Challenge language: any | javascript | html | cpp | csharp',e.challengeLanguage||'any')||'any';const boss=confirm('Bật Final Boss ở challenge cuối?');e.name=name.trim().slice(0,60)||e.name;e.description=desc.trim().slice(0,240);e.challengeCount=Math.floor(count);e.timeLimitMs=Math.floor(limit*1000);e.titleReward=reward.trim().slice(0,40)||e.titleReward;e.challengeLanguage=['any','javascript','html','cpp','csharp'].includes(lang)?lang:'any';e.boss=boss;saveEventStore(es);saveCloudEvent(e);renderAdmin();showToast('Event Draft đã cập nhật.','success')}
  async function adminTransition(btn,status){const id=btn.closest('.admin-event-card').querySelector('.admin-event-meta').dataset.event,es=eventStore(),e=es.find(x=>x.id===id);if(!e)return;if(status==='CLOSED'&&!roleIsOwner())return showToast('Chỉ Owner được Close Event để freeze leaderboard.','error');if(status==='CLOSED'&&!confirm(`Đóng Event ${e.name}? Leaderboard sẽ freeze.`))return;if(status==='CLOSED'){try{const top=await eventLeaderboard(e.id);e.finalTop=top.slice(0,10);e.finalizedAt=Date.now()}catch{e.finalTop=[]}}if(status==='ANNOUNCED'){e.resultsAnnouncedAt=Date.now();try{if(fbDb)await fbDb.collection('announcements').add({title:`${e.icon||'🎪'} ${e.name} — Final Results`,body:`Top 3: ${e.finalTop?.slice(0,3).map((r,i)=>`${['🥇','🥈','🥉'][i]} ${r.displayName||'Runner'} ${r.completed?formatMs(r.timeMs):'DNF'}`).join(' · ')||'Chưa có kết quả.'}`,type:'event',displayMode:'all',enabled:true,priority:10,startsAt:Date.now(),expiresAt:null,updatedAtMs:Date.now(),createdBy:state.currentUser?.uid||''})}catch(error){console.warn('event results announcement failed',error)}}if(status==='ARCHIVED')e.archivedAt=Date.now();e.status=status;saveEventStore(es);await saveCloudEvent(e);await v51WriteAudit('EVENT_'+status,{eventId:e.id});renderAdmin();showToast(`${e.name}: ${status}`,'success')}
  ...
  /* ==================== V5.1 ADMIN / OWNER EXPANSION ==================== */
  const V51 = {
    storage: {
      equippedRank: 'bugSpeedrunnerV51EquippedRank',
      adminTab: 'bugSpeedrunnerV51AdminTab'
    },
    audio: {
      admin: 'assets/audio/adminsong.mp3',
      cane: 'assets/audio/canesong.mp3'
    },
    ownerGif: 'assets/effects/blackhole.gif',
    adminTitle: 'Artmean',
    adminAchievement: 'Addmin',
    ownerTitle: 'OhMyBoss!?',
    ownerAchievement: 'TheBoss'
  };
  const SPECIAL_RANKS = [
    { id: 'admin-special', name: 'AdMIn', shortName: 'AdMIn', threshold: null, lucide: 'shield-check', description: 'Rank đặc quyền dành cho Admin. Không ảnh hưởng clear hoặc leaderboard.', effect: 'admin-flame', power: 10.5, color: '#ff5f67', specialRole: 'admin', audio: V51.audio.admin },
    { id: 'cane', name: 'Cane', threshold: null, lucide: 'black-hole', description: 'Rank đặc quyền tối cao của Owner. Không buff thành tích.', effect: 'cane-singularity', power: 11, color: '#e8e8ee', specialRole: 'owner', audio: V51.audio.cane }
  ];

  state.adminTab = safeJsonGet(V51.storage.adminTab, 'dashboard');
  state.announcementCache = [];
  state.v51AnnouncementsWired = false;

  const legacyEnsureUserShapeV51 = ensureUserShape;
  ensureUserShape = function(user) {
    const shaped = legacyEnsureUserShapeV51(user);
    const raw = user || {};
    return {
      ...shaped,
      titles: Array.isArray(raw.titles) ? raw.titles.slice(0, 100) : [],
      achievements: Array.isArray(raw.achievements) ? raw.achievements.slice(0, 100) : [],
      equippedTitle: String(raw.equippedTitle || ''),
      equippedRank: String(raw.equippedRank || ''),
      moderation: {
        leaderboardBlocked: Boolean(raw.moderation?.leaderboardBlocked),
        eventBlocked: Boolean(raw.moderation?.eventBlocked)
      },
      roleLabel: String(raw.roleLabel || '')
    };
  };

  function roleIsAdmin() { return state.adminRole === 'admin' || state.adminRole === 'owner'; }
  function roleIsOwner() { return state.adminRole === 'owner'; }
  function specialRankForRole(role) { return role === 'owner' ? SPECIAL_RANKS[1] : role === 'admin' ? SPECIAL_RANKS[0] : null; }
  function normalRankById(id) { return RANKS.find(r => r.id === id) || null; }
  function allSelectableRanks(user = state.currentUser) {
    const u = user || {};
    const clears = totalCorrect(u);
    const sameUser = state.currentUser?.uid && u.uid === state.currentUser.uid;
    const adminRole = sameUser ? state.adminRole : String(u.roleLabel || 'user');
    const normal = RANKS.filter(r => clears >= r.threshold || (sameUser && adminRole === 'owner'));
    const special = [];
    if (adminRole === 'admin' || adminRole === 'owner') special.push(SPECIAL_RANKS[0]);
    if (adminRole === 'owner') special.push(SPECIAL_RANKS[1]);
    return [...normal, ...special];
  }
  function equippedRankId(user = state.currentUser) {
    const raw = user?.equippedRank || localStorage.getItem(`${V51.storage.equippedRank}_${user?.uid || 'guest'}`) || '';
    if (!raw) return getRank(user).id;
    const rank = [...RANKS, ...SPECIAL_RANKS].find(r => r.id === raw);
    return allSelectableRanks(user).some(r => r.id === raw) && rank ? raw : getRank(user).id;
  }
  function displayedRankForUser(user = state.currentUser) {
    const id = equippedRankId(user);
    return [...RANKS, ...SPECIAL_RANKS].find(r => r.id === id) || getRank(user);
  }

  const legacyRankProgressV51 = rankProgress;
  rankProgress = function(user) {
    const actual = legacyRankProgressV51(user);
    if (!user || !roleIsOwner() || actual.current.id === 'emperor') return actual;
    return actual;
  };

  const legacyRankIconSvgV51 = rankIconSvg;
  rankIconSvg = function(rank, size = 'normal') {
    if (rank?.id === 'admin-special') {
      const css = `rank-symbol ${size} admin-special`;
      return `<svg class="${css}" viewBox="0 0 64 64" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M32 7 50 14v17c0 11-7 20-18 26C21 51 14 42 14 31V14L32 7Z"/><path d="M24 32c4-4 7-7 8-12 1 5 3 8 8 12-5 0-6 3-8 7-2-4-3-7-8-7Z"/></g></svg>`;
    }
    if (rank?.id === 'cane') {
      const css = `rank-symbol ${size} cane`;
      return `<svg class="${css}" viewBox="0 0 64 64" aria-hidden="true"><g fill="none" stroke="currentColor" stroke-width="2.3" stroke-linecap="round" stroke-linejoin="round"><circle cx="32" cy="32" r="13"/><path d="M32 5v14M32 45v14M5 32h14M45 32h14M13 13l10 10M41 41l10 10M51 13 41 23M23 41 13 51"/><path d="M25 27l7-5 7 5-7 11-7-11Z"/></g></svg>`;
    }
    return legacyRankIconSvgV51(rank, size);
  };

  const legacyAudioForRankV51 = audioForRank;
  audioForRank = function(rankId) {
    if (rankId === 'admin-special') return V51.audio.admin;
    if (rankId === 'cane') return V51.audio.cane;
    return legacyAudioForRankV51(rankId);
  };

  const legacyPlayRankAudioV51 = playRankAudio;
  playRankAudio = function(rankId) {
    const src = audioForRank(rankId);
    if (!src) return;
    stopRankAudio();
    rankAudio = new Audio(src);
    rankAudio.loop = true;
    rankAudio.volume = 0.42;
    rankAudio.play().catch(() => {});
  };

  function stopV51AudioAndModal() { stopRankAudio(); document.getElementById('rankReviewRoot')?.remove(); }

  function v51AvatarHtml(user, rank) {
    const name = user?.displayName || user?.name || user?.email?.split('@')[0] || 'Runner';
    const photo = user?.photoURL || '';
    const img = photo ? `<img src="${escapeHtml(photo)}" alt="" referrerpolicy="no-referrer" loading="lazy">` : `<span class="avatar-fallback">${escapeHtml(name.slice(0, 1).toUpperCase())}</span>`;
    const special = rank?.id === 'orbit'
      ? `<span class="profile-orbit-system"><i class="profile-orbit o1"></i><i class="profile-orbit o2"></i><i class="profile-orbit o3"></i><b class="orbit-planet p1"></b><b class="orbit-planet p2"></b><b class="orbit-planet p3"></b><i class="profile-orbit-star s1">✦</i><i class="profile-orbit-star s2">·</i><i class="profile-orbit-star s3">✦</i></span>`
      : rank?.id === 'admin-special'
        ? `<span class="admin-flame-orbit"><i>✦</i><i>·</i><i>✦</i></span>`
        : rank?.id === 'cane'
          ? `<span class="cane-energy"><i></i><i></i><i></i></span>`
          : '';
    return `<div class="profile-avatar ${rank?.id || ''}"><div class="profile-avatar-core">${img}</div>${special}</div>`;
  }

  function specialRankOwned(rank, user) {
    const role = user?.uid === state.currentUser?.uid ? state.adminRole : String(user?.roleLabel || 'user');
    return (rank.specialRole === 'admin' && (role === 'admin' || role === 'owner')) || (rank.specialRole === 'owner' && role === 'owner');
  }

  function v51RankReviewHtml(rank, user) {
    const displayName = user?.displayName || user?.name || user?.email?.split('@')[0] || 'Your Runner';
    const owned = allSelectableRanks(user).some(r => r.id === rank.id);
    const equipped = equippedRankId(user) === rank.id;
    const music = audioForRank(rank.id);
    return `<div class="rank-review-backdrop"><div class="rank-review-modal ${rank.id}">
      <div class="profile-topline"><span class="eyebrow">RANK REVIEW</span><button class="close-btn" id="rankReviewClose">✕</button></div>
      <div class="rank-review-stage ${rank.id}">${v51AvatarHtml(user, rank)}<div class="review-player"><div class="review-player-name ${rankNameClass(rank.id)}">${escapeHtml(displayName)}</div><div class="review-player-rank">${rankIconSvg(rank,'mini')} ${escapeHtml(rank.name)}</div><div class="review-owned-badge">${owned ? (equipped ? 'EQUIPPED' : 'OWNED / PREVIEW') : 'PREVIEW'}</div></div></div>
      <div class="rank-review-grid"><div><small>ICON</small><div class="review-icon ${rank.id}">${rankIconSvg(rank,'normal')}</div></div><div><small>NAME COLOR</small><div class="review-name-sample ${rankNameClass(rank.id)}">${escapeHtml(displayName)}</div></div><div><small>EFFECT</small><div class="review-effect-badge ${rank.id}">POWER ${rank.power || 0}/10 · ${escapeHtml(rank.effect || 'none')}</div></div><div><small>REQUIREMENT</small><div class="review-threshold">${rank.specialRole ? `ROLE: ${rank.specialRole.toUpperCase()}` : `${rank.threshold} clears`}</div></div></div>
      <div class="rank-review-footer"><span>${escapeHtml(rank.description)}</span>${music ? `<span class="audio-note">♫ ${escapeHtml(music.split('/').pop())}</span>` : ''}</div>
      ${owned ? `<button class="primary-btn rank-review-equip" id="rankReviewEquipBtn">${equipped ? '✓ Đang trang bị' : '⚡ Trang bị rank này'}</button>` : ''}
    </div></div>`;
  }

  openRankReview = function(rankId) {
    const rank = [...RANKS, ...SPECIAL_RANKS].find(r => r.id === rankId) || RANKS[0];
    const user = state.currentUser;
    stopRankAudio();
    document.getElementById('rankReviewRoot')?.remove();
    const root = document.createElement('div'); root.id = 'rankReviewRoot'; root.innerHTML = v51RankReviewHtml(rank, user); document.body.appendChild(root);
    $('rankReviewClose').onclick = closeRankReview;
    root.querySelector('.rank-review-backdrop').onclick = e => { if (e.target.classList.contains('rank-review-backdrop')) closeRankReview(); };
    $('rankReviewEquipBtn')?.addEventListener('click', async () => { await v51EquipRank(rank.id); renderRank(); closeRankReview(); });
    if (audioForRank(rank.id)) playRankAudio(rank.id);
  };
  closeRankReview = function() { stopRankAudio(); document.getElementById('rankReviewRoot')?.remove(); };

  async function v51EquipRank(rankId) {
    const user = state.currentUser;
    if (!user) return showToast('Đăng nhập để trang bị rank.', 'error');
    const allowed = allSelectableRanks(user).some(r => r.id === rankId);
    if (!allowed) return showToast('Rank này chưa được mở khóa cho tài khoản.', 'error');
    const rank = [...RANKS, ...SPECIAL_RANKS].find(r => r.id === rankId);
    if (!rank) return;
    state.currentUser.equippedRank = rankId;
    localStorage.setItem(`${V51.storage.equippedRank}_${user.uid}`, rankId);
    try {
      if (fbDb) await fbDb.collection('users').doc(user.uid).update({ equippedRank: rankId, updatedAt: firebase.firestore.FieldValue.serverTimestamp() });
    } catch (error) {
      showToast(`Chưa đồng bộ equipped rank: ${error.message}`, 'error');
    }
    renderHeader();
    showToast(`⚡ Đã trang bị ${rank.name}.`, 'success');
  }


  buildRunnerStats = function() {
    return Array.from(state.leaderboardUsers.entries()).filter(([key, raw]) => !ensureUserShape(raw).moderation?.leaderboardBlocked).map(([key, raw]) => {
      const user = { key, ...ensureUserShape(raw) };
      const name = user.displayName || user.email?.split('@')[0] || 'Runner';
      return { key, name, user, rank: displayedRankForUser(user), actualRank: getRank(user), bestMs: bestOverallMs(user), best: bestOverall(user), clears: totalCorrect(user), runs: Number(user.stats.totalRuns || 0) };
    });
  };

  function renderRankV51() {
    const user = state.currentUser, actual = getRank(user), equipped = displayedRankForUser(user), pr = rankProgress(user), p = v5Progress();
    const cards = RANKS.map((r, i) => {
      const n = RANKS[i + 1]; const owned = Boolean(user && (roleIsOwner() || totalCorrect(user) >= r.threshold));
      const pct = user ? (r.id === 'emperor' ? 100 : Math.max(0, Math.min(100, ((totalCorrect(user) - r.threshold) / Math.max(1, (n?.threshold || r.threshold + 1) - r.threshold)) * 100))) : 0;
      return `<article class="rank-card ${r.id} rank-power-${r.power || 0} ${r.id === actual.id ? 'current' : ''} ${r.id === equipped.id ? 'equipped' : ''}"><button type="button" class="rank-review-hit" data-rank-id="${r.id}"><div class="rank-card-aura"></div><div class="rank-top"><div class="rank-icon ${r.id}">${rankIconSvg(r,'normal')}</div><div><div class="rank-name ${rankNameClass(r.id)}">${escapeHtml(r.name)}</div><div class="rank-requirement">${r.threshold === 0 ? 'Bắt đầu' : `${r.threshold} speedrun đúng`}</div></div></div>${r.id === actual.id ? '<span class="rank-current-tag">ACTUAL</span>' : ''}${r.id === equipped.id ? '<span class="rank-equipped-tag">EQUIPPED</span>' : ''}<p class="rank-copy">${escapeHtml(r.description)}</p><div class="rank-bar"><span style="width:${pct}%"></span></div><span class="rank-review-label">REVIEW ↗</span></button>${owned ? `<button class="secondary-btn rank-equip-btn" data-equip-rank="${r.id}">${r.id === equipped.id ? '✓ Equipped' : 'Trang bị'}</button>` : '<span class="rank-locked-note">🔒 Locked</span>'}</article>`;
    }).join('');
    const specialCards = SPECIAL_RANKS.map(r => { const owned = specialRankOwned(r, user); const eq = equipped.id === r.id; return `<article class="rank-card special-rank ${r.id} ${eq ? 'equipped' : ''}"><button type="button" class="rank-review-hit" data-rank-id="${r.id}"><div class="rank-card-aura"></div><div class="rank-top"><div class="rank-icon ${r.id}">${rankIconSvg(r,'normal')}</div><div><div class="rank-name ${rankNameClass(r.id)}">${escapeHtml(r.name)}</div><div class="rank-requirement">ROLE EXCLUSIVE</div></div></div>${eq ? '<span class="rank-equipped-tag">EQUIPPED</span>' : ''}<p class="rank-copy">${escapeHtml(r.description)}</p><span class="rank-review-label">REVIEW ↗</span></button>${owned ? `<button class="secondary-btn rank-equip-btn" data-equip-rank="${r.id}">${eq ? '✓ Equipped' : 'Trang bị'}</button>` : '<span class="rank-locked-note">🔒 Restricted</span>'}</article>`; }).join('');
    const mini = ACHIEVEMENT_DEFS.slice(0, 8).map(a => { const v = a.metric === 'clears' ? totalCorrect(user) : a.metric === 'bestStreak' ? state.bestStreak : a.metric === 'roleAdmin' ? ((state.adminRole === 'admin' || state.adminRole === 'owner') ? 1 : 0) : a.metric === 'roleOwner' ? (state.adminRole === 'owner' ? 1 : 0) : (p.stats[a.metric] || 0); return `<div class="achievement ${p.unlocked.includes(a.id) ? 'unlocked' : ''}"><div class="a-icon">${a.icon}</div><strong>${a.name}</strong><small>${Math.min(a.target,v)}/${a.target}</small></div>`; }).join('');
    view.innerHTML = `<section class="view page-shell"><div class="page-head"><div class="page-title"><button class="nav-btn back-btn" id="rankBackBtn">← Menu</button><div><h2>Rank System</h2><div class="page-subtitle">11 rank tiến trình + 2 rank đặc quyền • Equipped Rank</div></div></div><div class="page-head-actions"><button class="secondary-btn" id="rankAchievementBtn">🏅 Achievements</button><button class="primary-btn" id="rankSpeedrunBtn">⚡ Speedrun</button></div></div><div class="page-content"><div class="rank-hero"><div class="rank-progress-card ${actual.id}"><div class="rank-progress-head"><span class="eyebrow">ACTUAL RANK</span><strong>${user ? `${totalCorrect(user)} clears` : 'Guest'}</strong></div><div class="rank-progress-main"><span class="big-icon ${equipped.id}">${rankIconSvg(equipped,'normal')}</span><div><strong class="${rankNameClass(equipped.id)}">${escapeHtml(equipped.name)}</strong><br><span>Hiển thị theo Equipped Rank • Actual: ${escapeHtml(actual.name)}</span></div></div><div class="progress-track"><span style="width:${pr.pct}%"></span></div><div class="rank-progress-note">${pr.next ? `Còn ${pr.remaining} clear để lên ${pr.next.name}.` : '👑 Đế vương — MAX RANK.'}</div></div><div class="rank-tip-card"><h3>✨ Rank Review</h3><p>Icon đứng yên; aura/particle mới chuyển động. Chọn rank để xem preview và trang bị rank đã mở.</p><button class="secondary-btn" id="openCurrentReview">Review ${escapeHtml(equipped.name)}</button></div></div><div class="rank-grid rank-grid-v5">${cards}</div><div class="rank-section-head"><div><span class="eyebrow">SPECIAL ACCESS RANKS</span><h3>Rank đặc quyền</h3></div><span class="muted">Không cộng clear, không buff leaderboard</span></div><div class="rank-grid rank-grid-v5 special-rank-grid">${specialCards}</div><div class="rank-section-head"><div><span class="eyebrow">ACHIEVEMENTS</span><h3>Thành tựu</h3></div><button class="link-btn" id="openAllAchievements">Xem tất cả →</button></div><div class="achievement-grid">${mini}</div></div></section>`;
    $('rankBackBtn').onclick=()=>go('menu'); $('rankSpeedrunBtn').onclick=()=>go('speedrun'); $('rankAchievementBtn').onclick=()=>go('achievements'); $('openAllAchievements').onclick=()=>go('achievements'); $('openCurrentReview').onclick=()=>openRankReview(equipped.id);
    document.querySelectorAll('.rank-review-hit').forEach(b=>b.onclick=()=>openRankReview(b.dataset.rankId));
    document.querySelectorAll('.rank-equip-btn').forEach(b=>b.onclick=()=>v51EquipRank(b.dataset.equipRank).then(renderRank));
  }
  renderRank = renderRankV51;

  function profileRankForUserV51(user) { return displayedRankForUser(user); }
  const legacyOpenProfileV51 = openProfile;
  openProfile = function(key, self = false) {
    const map = state.leaderboardUsers; const user = map.get(key) || (state.currentUser?.key === key ? state.currentUser : null);
    if (!user) return legacyOpenProfileV51(key, self);
    stopRankAudio(); document.getElementById('profileRoot')?.remove();
    const rank = profileRankForUserV51(user); const actual = getRank(user); const isSelf = self || state.currentUser?.key === key;
    const titles = Array.isArray(user.titles) ? user.titles : v5Titles();
    const equippedTitle = user.equippedTitle || (isSelf ? state.equippedTitle : '') || localStorage.getItem(`bugSpeedrunnerEquippedTitle_${v5Uid()}`) || '';
    const root = document.createElement('div'); root.id='profileRoot';
    root.innerHTML = `<div class="profile-backdrop"><div class="profile-modal v51-profile ${rank.id}"><div class="profile-topline"><div><span class="eyebrow">RUNNER PROFILE</span><div class="profile-security-note">Actual ${escapeHtml(actual.name)} · Equipped ${escapeHtml(rank.name)}</div></div><button class="close-btn" id="profileCloseBtn">✕</button></div><div class="profile-hero ${rank.id}">${v51AvatarHtml(user, rank)}<div class="profile-hero-copy"><div class="profile-name ${rankNameClass(rank.id)}">${escapeHtml(user.displayName || user.name || 'Runner')}</div><div class="profile-rank-label">${rankIconSvg(rank,'mini')} ${escapeHtml(rank.name)} <span class="equipped-pill">EQUIPPED</span></div>${equippedTitle ? `<div class="profile-title-line">🏷️ ${escapeHtml(equippedTitle)}</div>` : ''}</div></div><div class="profile-grid"><div><small>CLEARS</small><strong>${totalCorrect(user)}</strong></div><div><small>RUNS</small><strong>${getTotalRuns(user)}</strong></div><div><small>SUCCESS RATE</small><strong>${getTotalRuns(user) ? Math.round((totalCorrect(user)/getTotalRuns(user))*100) : 0}%</strong></div><div><small>BEST TIME</small><strong>${user.bestTimeMs ? formatMs(user.bestTimeMs) : '—'}</strong></div></div><div class="profile-progress"><div><span>ACTUAL RANK</span><strong>${escapeHtml(actual.name)}</strong></div><div class="progress-track"><span style="width:${rankProgress(user).pct}%"></span></div></div>${titles.length ? `<div class="profile-showcase"><div class="profile-section-title">TITLE SHOWCASE</div><div class="profile-title-chips">${titles.slice(0,3).map(t=>`<span class="profile-title-chip ${escapeHtml(t.rarity||'common')}">🏷️ ${escapeHtml(t.name || t)}</span>`).join('')}</div></div>` : ''}<div class="profile-history"><div class="profile-section-title">RECENT RUNS</div>${(user.history||[]).slice(0,8).map(item=>`<div class="profile-history-row"><span class="history-result ${item.correct?'ok':'bad'}">${item.correct?'CLEAR':item.timedOut?'TIMEOUT':'FAIL'}</span><span>${escapeHtml(item.title)}</span><span>${formatMs(item.ms)}</span></div>`).join('') || '<div class="ai-empty">Chưa có run nào.</div>'}</div>${isSelf ? `<div class="profile-actions"><button class="secondary-btn" id="profileRankEquipBtn">🎒 Đổi Equipped Rank</button><button class="secondary-btn" id="profileSettingsBtn">⚙ Settings</button><button class="danger-btn" id="profileLogoutBtn">Đăng xuất</button></div>` : ''}</div></div>`;
    document.body.appendChild(root);
    $('profileCloseBtn').onclick=closeProfile; root.querySelector('.profile-backdrop').onclick=e=>{if(e.target.classList.contains('profile-backdrop'))closeProfile()};
    $('profileRankEquipBtn')?.addEventListener('click',()=>{closeProfile();go('rank');}); $('profileSettingsBtn')?.addEventListener('click',()=>{closeProfile();openSettings();}); $('profileLogoutBtn')?.addEventListener('click',logout);
    if (isSelf && audioForRank(rank.id)) playRankAudio(rank.id);
  };
  closeProfile = function() { stopRankAudio(); document.getElementById('profileRoot')?.remove(); };

  const legacyRenderAchievementsV51 = renderAchievements;
  renderAchievements = function() {
    const p=v5Progress(), u=state.currentUser, ts=v5Titles();
    const titleList=[...new Map([...ts, ...(u?.titles||[]).map(t=>typeof t==='string'?{name:t,source:'admin',rarity:'special'}:t)].map(t=>[t.name,t])).values()];
    const html=ACHIEVEMENT_DEFS.map(a=>{const v=a.metric==='clears'?totalCorrect(u):a.metric==='bestStreak'?state.bestStreak:a.metric==='roleAdmin'?((state.adminRole==='admin'||state.adminRole==='owner')?1:0):a.metric==='roleOwner'?(state.adminRole==='owner'?1:0):(p.stats[a.metric]||0);const unlocked=p.unlocked.includes(a.id)||(u?.achievements||[]).includes(a.id);return `<article class="achievement-detail ${unlocked?'unlocked':''}"><div class="achievement-badge">${a.icon}</div><div class="achievement-main"><div class="achievement-rarity ${a.rarity}">${a.rarity}</div><h3>${escapeHtml(a.name)}</h3><p>${escapeHtml(a.desc)}</p><div class="achievement-progress"><span style="width:${Math.min(100,(v/a.target)*100)}%"></span></div><small>${Math.min(v,a.target)}/${a.target} · Reward: <strong>${escapeHtml(a.title)}</strong></small></div>${unlocked?'<div class="achievement-unlocked">UNLOCKED</div>':''}</article>`}).join('');
    const titles=titleList.length?titleList.map(t=>`<div class="title-card ${escapeHtml(t.rarity||'common')}"><span>🏷️</span><div><strong>${escapeHtml(t.name||t)}</strong><small>${escapeHtml(t.rarity||'common')}</small></div><button class="small-tool equip-title" data-title="${escapeHtml(t.name||t)}">${state.equippedTitle===String(t.name||t)?'Equipped':'Equip'}</button></div>`).join(''):'<div class="ai-empty">Chưa có title.</div>';
    view.innerHTML=`<section class="view page-shell"><div class="page-head"><div class="page-title"><button class="nav-btn back-btn" id="achBackBtn">← Menu</button><div><h2>Achievements & Titles</h2><div class="page-subtitle">Hoàn thành thành tựu → nhận title → trang bị.</div></div></div><button class="primary-btn" id="achEventBtn">⚡ Events</button></div><div class="page-content"><div class="achievement-detail-grid">${html}</div><div class="section-divider"></div><div class="rank-section-head"><div><span class="eyebrow">TITLE COLLECTION</span><h3>Title collection</h3></div><span class="muted">${titleList.length} titles</span></div><div class="title-grid">${titles}</div></div></section>`;
    $('achBackBtn').onclick=()=>go('menu'); $('achEventBtn').onclick=()=>go('events'); document.querySelectorAll('.equip-title').forEach(b=>b.onclick=()=>v51EquipTitle(b.dataset.title));
  };

  async function v51EquipTitle(title) {
    if (!state.currentUser) return;
    const owned = v5Titles().some(t=>(t.name||t)===title) || (state.currentUser.titles||[]).some(t=>(t.name||t)===title);
    if (!owned) return showToast('Title này chưa được sở hữu.', 'error');
    state.equippedTitle=title; localStorage.setItem(`bugSpeedrunnerEquippedTitle_${v5Uid()}`, title); state.currentUser.equippedTitle=title;
    try { if(fbDb) await fbDb.collection('users').doc(state.currentUser.uid).update({equippedTitle:title,updatedAt:firebase.firestore.FieldValue.serverTimestamp()}); } catch(error){showToast(`Không đồng bộ title: ${error.message}`,'error')}
    renderAchievements(); renderHeader(); showToast(`🏷️ Đã trang bị ${title}`,'success');
  }

  const legacyRenderHeaderV51 = renderHeader;
  renderHeader = function() {
    if (state.currentUser) {
      state.equippedTitle = state.currentUser.equippedTitle || localStorage.getItem(`bugSpeedrunnerEquippedTitle_${v5Uid()}`) || '';
      const savedRank = state.currentUser.equippedRank || localStorage.getItem(`${V51.storage.equippedRank}_${state.currentUser.uid}`) || '';
      state.currentUser.equippedRank = savedRank || getRank(state.currentUser).id;
    }
    legacyRenderHeaderV51();
    const top=document.querySelector('.top-actions'); if(!top) return;
    const activeCount=activeAnnouncementsV51().length;let nb=document.getElementById('v51NotificationBtn');if(!nb){nb=document.createElement('button');nb.id='v51NotificationBtn';nb.className='nav-btn notification-nav-btn';nb.onclick=()=>openNotificationCenterV51();top.appendChild(nb)}nb.textContent=`🔔 ${activeCount?activeCount:''}`.trim();nb.title=activeCount?`${activeCount} thông báo`:'Notifications';
    if(!document.getElementById('topEventsBtn')){const b=document.createElement('button');b.id='topEventsBtn';b.className='nav-btn';b.textContent='Events';b.onclick=()=>go('events');top.appendChild(b)}
    if(!document.getElementById('topAchievementsBtn')){const b=document.createElement('button');b.id='topAchievementsBtn';b.className='nav-btn';b.textContent='🏅 Ach';b.onclick=()=>go('achievements');top.appendChild(b)}
    if(roleIsAdmin()){let b=document.getElementById('topAdminBtn');if(!b){b=document.createElement('button');b.id='topAdminBtn';b.className='nav-btn admin-nav-btn';b.textContent=state.adminRole==='owner'?'👑 Owner':'🛡️ Admin';b.onclick=()=>go('admin');top.appendChild(b)}}else document.getElementById('topAdminBtn')?.remove();
  };

  function notificationPanelHtmlV51(){const list=activeAnnouncementsV51().slice(0,8);return `<div class="v51-notification-popover" id="v51NotificationPopover"><div class="v51-notification-head"><strong>Notifications</strong><span>${list.length}</span></div>${list.length?list.map(a=>`<button class="v51-notification-item" data-notification-id="${escapeHtml(a.id||'')}"><span class="announcement-icon">${a.type==='event'?'🔥':a.type==='maintenance'?'🛠':a.type==='reward'?'🎁':a.type==='important'?'🚨':'📢'}</span><span><strong>${escapeHtml(a.title||'Announcement')}</strong><small>${escapeHtml(a.body||'')}</small></span></button>`).join(''):'<div class="v51-notification-empty">Không có thông báo mới.</div>'}</div>`;}
  function openNotificationCenterV51(){document.getElementById('v51NotificationPopover')?.remove();const b=document.getElementById('v51NotificationBtn');if(!b)return;b.insertAdjacentHTML('afterend',notificationPanelHtmlV51());document.querySelectorAll('.v51-notification-item').forEach(item=>item.onclick=()=>{document.getElementById('v51NotificationPopover')?.remove();const id=item.dataset.notificationId;if(id){localStorage.setItem(`bugSpeedrunnerDismissedAnnouncement_${id}`,'1');renderMenu();}});setTimeout(()=>document.addEventListener('click',function close(e){if(!e.target.closest('#v51NotificationPopover')&&!e.target.closest('#v51NotificationBtn')){document.getElementById('v51NotificationPopover')?.remove();document.removeEventListener('click',close)}},{once:true}),0)}

  function activeAnnouncementsV51() {
    const now=Date.now(); return (state.announcementCache||[]).filter(a=>a.enabled!==false && (!a.startsAt || a.startsAt<=now) && (!a.expiresAt || a.expiresAt>now)).sort((a,b)=>(Number(b.priority)||0)-(Number(a.priority)||0));
  }
  function announcementHtmlV51() { const a=activeAnnouncementsV51().find(x=>!localStorage.getItem(`bugSpeedrunnerDismissedAnnouncement_${x.id}`)); if(!a)return ''; return `<div class="announcement-bar ${escapeHtml(a.type||'announcement')}"><span class="announcement-icon">${a.type==='event'?'🔥':a.type==='maintenance'?'🛠':a.type==='reward'?'🎁':a.type==='important'?'🚨':'📢'}</span><div><strong>${escapeHtml(a.title||'Announcement')}</strong><span>${escapeHtml(a.body||'')}</span></div><button class="announcement-close" type="button" data-announcement-close="${escapeHtml(a.id||'')}">×</button></div>`; }
  function wireAnnouncementBar(){document.querySelectorAll('[data-announcement-close]').forEach(b=>b.onclick=()=>{const key=`bugSpeedrunnerDismissedAnnouncement_${b.dataset.announcementClose}`;localStorage.setItem(key,'1');b.closest('.announcement-bar')?.remove()})}
  const legacyRenderMenuV51 = renderMenu;
  renderMenu = function(){ legacyRenderMenuV51(); const shell=document.querySelector('.menu-view .menu-inner'); if(!shell)return; const bar=announcementHtmlV51(); if(bar && !shell.querySelector('.announcement-bar')){shell.insertAdjacentHTML('afterbegin',bar);wireAnnouncementBar()} };

  function v51AdminTabs() {
    return [['dashboard','📊 Dashboard'],['players','👥 Players'],['events','🎪 Events'],['announcements','📢 Announcements'],['security','🛡️ Security Center'],['testlab','🧪 Test Lab']];
  }
  function v51AdminNav(){return `<nav class="admin-nav-tabs">${v51AdminTabs().map(([id,label])=>`<button class="admin-tab ${state.adminTab===id?'active':''}" data-admin-tab="${id}">${label}</button>`).join('')}</nav>`}
  function adminStatsV51(es){return {events:es.length,live:es.filter(e=>e.status==='LIVE').length,drafts:es.filter(e=>e.status==='DRAFT').length,archived:es.filter(e=>e.status==='ARCHIVED').length,players:state.leaderboardUsers.size,achievements:ACHIEVEMENT_DEFS.length,titles:new Set([...ACHIEVEMENT_DEFS.map(a=>a.title),V51.adminTitle,V51.ownerTitle]).size};}
  function adminDashboardHtmlV51(es){const s=adminStatsV51(es);return `<div class="admin-dashboard v51-admin-dashboard"><div class="admin-stat"><small>PLAYERS</small><strong>${s.players}</strong></div><div class="admin-stat"><small>EVENTS</small><strong>${s.events}</strong></div><div class="admin-stat"><small>LIVE</small><strong>${s.live}</strong></div><div class="admin-stat"><small>DRAFTS</small><strong>${s.drafts}</strong></div><div class="admin-stat"><small>ARCHIVED</small><strong>${s.archived}</strong></div><div class="admin-stat"><small>ACHIEVEMENTS</small><strong>${s.achievements}</strong></div><div class="admin-stat"><small>TITLES</small><strong>${s.titles}</strong></div><div class="admin-stat"><small>ROLE</small><strong>${escapeHtml(state.adminRole.toUpperCase())}</strong></div></div><div class="admin-insight-grid"><article class="admin-insight"><span class="eyebrow">SYSTEM</span><h3>Cloud Arena</h3><p>Role thật lấy từ Firebase ID token. Frontend chỉ là lớp điều khiển.</p><span class="status-pill ${state.cloudStatus}">${escapeHtml(state.cloudStatus)}</span></article><article class="admin-insight"><span class="eyebrow">EQUIPPED RANK</span><h3>${escapeHtml(displayedRankForUser(state.currentUser).name)}</h3><p>Actual: ${escapeHtml(getRank(state.currentUser).name)}</p></article><article class="admin-insight"><span class="eyebrow">SPECIAL ACCESS</span><h3>AdMIn · Cane</h3><p>Rank cosmetic riêng cho Admin/Owner, không buff clear.</p></article></div>`;}
  function playersAdminHtmlV51(){const rows=[...state.leaderboardUsers.values()].sort((a,b)=>(totalCorrect(b)-totalCorrect(a))).slice(0,200);return `<div class="admin-section-head"><div><span class="eyebrow">PLAYER MANAGEMENT</span><h3>Tài khoản & moderation</h3></div><input id="adminPlayerSearch" class="admin-search" placeholder="Tìm tên / email / UID..."></div><div class="admin-player-table-wrap"><table class="admin-player-table"><thead><tr><th>Player</th><th>Actual</th><th>Equipped</th><th>Clears</th><th>Role</th><th>Moderation</th><th>Actions</th></tr></thead><tbody>${rows.map(u=>{const role=u.uid===state.currentUser?.uid?state.adminRole:(u.roleLabel||'user');const blocked=u.moderation?.leaderboardBlocked||u.moderation?.eventBlocked;return `<tr data-player-row="${escapeHtml(u.key||u.uid)}"><td><div class="admin-player-cell">${u.photoURL?`<img src="${escapeHtml(u.photoURL)}" alt="" referrerpolicy="no-referrer">`:'<span class="mini-avatar">?</span>'}<div><strong>${escapeHtml(u.displayName||'Runner')}</strong><small>${escapeHtml(u.email||u.uid||'')}</small></div></div></td><td><span class="rank-inline ${getRank(u).id}">${rankIconSvg(getRank(u),'mini')} ${escapeHtml(getRank(u).name)}</span></td><td><span class="rank-inline ${displayedRankForUser(u).id}">${rankIconSvg(displayedRankForUser(u),'mini')} ${escapeHtml(displayedRankForUser(u).name)}</span></td><td>${totalCorrect(u)}</td><td><span class="role-mini ${role}">${escapeHtml(role.toUpperCase())}</span></td><td>${blocked?'<span class="danger-text">BLOCKED</span>':'<span class="success-text">OK</span>'}</td><td class="admin-player-actions"><button class="small-tool view-admin-player" data-player="${escapeHtml(u.key||u.uid)}">View</button>${roleIsOwner()&&u.uid!==state.currentUser?.uid?`<button class="small-tool role-btn" data-player="${escapeHtml(u.key||u.uid)}">Role</button><button class="small-tool grant-title" data-player="${escapeHtml(u.key||u.uid)}">Title</button><button class="small-tool lb-block-btn" data-player="${escapeHtml(u.key||u.uid)}">LB ${u.moderation?.leaderboardBlocked?'Unlock':'Lock'}</button><button class="small-tool event-block-btn" data-player="${escapeHtml(u.key||u.uid)}">Event ${u.moderation?.eventBlocked?'Unlock':'Lock'}</button><button class="small-tool reset-stats-btn" data-player="${escapeHtml(u.key||u.uid)}">Reset</button>`:''}</td></tr>`}).join('')}</tbody></table></div><div class="admin-action-note">Role Admin/Owner được cấp từ trusted server/script. Nút Role sẽ chuẩn bị thao tác an toàn thay vì giả vờ cấp quyền bằng frontend.</div>`;}

  async function ownerGrantOrRevokeAdminFlow() {
    if(!roleIsOwner())return showToast('Chỉ Owner mới được quản lý Admin.','error');
    const uid=prompt('UID tài khoản cần cấp/thu hồi Admin:',''); if(!uid)return;
    const action=confirm('OK = cấp Admin\nCancel = thu hồi Admin');
    const role=action?'admin':'user';
    const command=`node grant-role.cjs\n# UID: ${uid}\n# ROLE: ${role}`;
    await navigator.clipboard?.writeText(command).catch(()=>{});
    showToast(`Đã chuẩn bị lệnh ${role==='admin'?'cấp':'thu hồi'} Admin cho UID ${uid}. Đã copy nếu trình duyệt cho phép.`,'success');
    await v51WriteAudit('ROLE_COMMAND_PREPARED',{uid,targetRole:role});
  }
  async function ownerGrantOrRevokeAdminFlowForUid(uid) {
    if(!roleIsOwner()) return showToast('Chỉ Owner mới được quản lý Admin.','error');
    if(!uid) return;
    const action = confirm('OK = cấp Admin\nCancel = thu hồi Admin');
    const role = action ? 'admin' : 'user';
    const command = `node grant-role.cjs\n# UID: ${uid}\n# ROLE: ${role}`;
    await navigator.clipboard?.writeText(command).catch(()=>{});
    showToast(`Đã chuẩn bị lệnh ${role==='admin'?'cấp':'thu hồi'} Admin cho ${uid}.`, 'success');
    try { if(fbDb) await fbDb.collection('adminRegistry').doc(uid).set({uid,role,lastActionBy:state.currentUser?.uid||'',updatedAt:firebase.firestore.FieldValue.serverTimestamp()},{merge:true}); } catch {}
    await v51WriteAudit('ROLE_COMMAND_PREPARED',{uid,targetRole:role});
  }


  async function ownerModifyPlayer(uid, mode){
    if(!roleIsOwner()||!fbDb)return showToast('Chỉ Owner mới có quyền này.','error');
    const ref=fbDb.collection('users').doc(uid); const snap=await ref.get(); if(!snap.exists)return showToast('Không tìm thấy user.','error'); const u=ensureUserShape({key:uid,...snap.data()});
    if(mode==='title'){const title=prompt('Title cần cấp/thu hồi:',V51.adminTitle);if(!title)return;const has=(u.titles||[]).some(t=>(t.name||t)===title);const next=has?(u.titles||[]).filter(t=>(t.name||t)!==title):[...(u.titles||[]),{name:title,source:'owner-grant',rarity:'special',at:Date.now()}];await ref.update({titles:next,updatedAt:firebase.firestore.FieldValue.serverTimestamp()});showToast(`${has?'Đã thu hồi':'Đã cấp'} title ${title}.`,'success');}
    if(mode==='moderation'){const lb=!u.moderation?.leaderboardBlocked, ev=!u.moderation?.eventBlocked;await ref.update({moderation:{leaderboardBlocked:lb,eventBlocked:ev},updatedAt:firebase.firestore.FieldValue.serverTimestamp()});showToast(`Moderation: Leaderboard ${lb?'khóa':'mở'} · Event ${ev?'khóa':'mở'}.`,'success');}
    if(mode==='leaderboardBlock'){const lb=!u.moderation?.leaderboardBlocked;await ref.update({moderation:{leaderboardBlocked:lb,eventBlocked:Boolean(u.moderation?.eventBlocked)},updatedAt:firebase.firestore.FieldValue.serverTimestamp()});showToast(`Leaderboard ${lb?'đã khóa':'đã mở'}.`,'success');}
    if(mode==='eventBlock'){const ev=!u.moderation?.eventBlocked;await ref.update({moderation:{leaderboardBlocked:Boolean(u.moderation?.leaderboardBlocked),eventBlocked:ev},updatedAt:firebase.firestore.FieldValue.serverTimestamp()});showToast(`Event ${ev?'đã khóa':'đã mở'}.`,'success');}
    if(mode==='reset'){if(!confirm(`Reset toàn bộ thống kê của ${u.displayName}?`))return;await ref.update({totalRuns:0,totalCorrect:0,bestTimeMs:null,hasClear:false,records:{},history:[],updatedAt:firebase.firestore.FieldValue.serverTimestamp()});showToast('Đã reset thống kê.','success');}
    await v51WriteAudit('OWNER_PLAYER_ACTION',{uid,mode});
    renderAdmin();
  }

  async function v51WriteAudit(action,meta={}){try{if(fbDb&&state.currentUser&&roleIsAdmin()){await fbDb.collection('auditLogs').add({actorUid:state.currentUser.uid,actorRole:state.adminRole,action,meta,createdAt:firebase.firestore.FieldValue.serverTimestamp()})}}catch(error){console.warn('audit log failed',error)}}

  function eventsAdminHtmlV51(es){return `<div class="admin-section-head"><div><span class="eyebrow">EVENT MANAGER</span><h3>Lifecycle Control</h3><p class="muted">Draft → Publish → Live → Close → Announce → Archive → Duplicate</p></div><div class="admin-inline-actions"><button class="primary-btn" id="newEventBtn">＋ New Event</button><button class="secondary-btn" id="refreshAdminBtn">↻ Refresh</button></div></div><div class="admin-event-list v51-event-list">${es.map(e=>`<article class="admin-event-card v51-event-card"><div class="v51-event-main"><span class="event-status ${String(e.status).toLowerCase()}">${escapeHtml(e.status)}</span><h3>${escapeHtml(e.icon+' '+e.name)} <small>v${e.version||1}</small></h3><p>${escapeHtml(e.description||'')}</p><div class="admin-event-metrics"><span>${e.challengeCount||0} challenges</span><span>${formatMs(e.timeLimitMs||0)} limit</span><span>Reward: ${escapeHtml(e.titleReward||'—')}</span></div></div><div class="admin-event-actions">${e.status==='DRAFT'?'<button class="secondary-btn edit-event">✏️ Edit</button><button class="secondary-btn publish-event">📢 Publish</button>':''}${e.status==='PUBLISHED'?'<button class="primary-btn start-live-event">🔴 Start Live</button>':''}${e.status==='LIVE'?'<button class="danger-btn close-event">🔒 Close</button><button class="secondary-btn monitor-event">📊 Monitor</button>':''}${e.status==='CLOSED'?'<button class="primary-btn announce-event">📣 Announce</button>':''}${e.status==='ANNOUNCED'?'<button class="secondary-btn archive-event">📦 Archive</button>':''}<button class="secondary-btn duplicate-event">♻ Duplicate</button></div><div class="admin-event-meta" data-event="${escapeHtml(e.id)}">ID ${escapeHtml(e.id)} · status ${escapeHtml(e.status)} · ${e.finalTop?.length||0} final rows</div></article>`).join('')}</div>`;}

  function announcementsAdminHtmlV51(){const list=(state.announcementCache||[]).slice().sort((a,b)=>(b.updatedAtMs||0)-(a.updatedAtMs||0));return `<div class="admin-section-head"><div><span class="eyebrow">ANNOUNCEMENT SYSTEM</span><h3>Thông báo</h3></div><button class="primary-btn" id="newAnnouncementBtn">＋ New Announcement</button></div><div class="announcement-admin-grid">${list.length?list.map(a=>`<article class="announcement-admin-card ${escapeHtml(a.type||'announcement')}"><div><span class="event-status">${escapeHtml(a.type||'announcement')}</span><h3>${escapeHtml(a.title||'Announcement')}</h3><p>${escapeHtml(a.body||'')}</p><small>${a.expiresAt?`Expires ${new Date(a.expiresAt).toLocaleString()}`:'No expiry'}</small></div><div class="admin-event-actions"><button class="secondary-btn edit-announcement" data-ann="${escapeHtml(a.id)}">✏️ Edit</button><button class="danger-btn delete-announcement" data-ann="${escapeHtml(a.id)}">🗑 Delete</button></div></article>`).join(''):'<div class="ai-empty">Chưa có announcement.</div>'}</div>`;}
  async function createOrEditAnnouncement(id=''){
    const existing=(state.announcementCache||[]).find(a=>a.id===id)||{}; const title=prompt('Tiêu đề:',existing.title||'NEW ANNOUNCEMENT');if(title===null)return;const body=prompt('Nội dung:',existing.body||'');if(body===null)return;const type=prompt('Loại: announcement | event | maintenance | reward | important',existing.type||'announcement')||'announcement';const mode=prompt('Hiển thị: all | onopen | popup | notification',existing.displayMode||'all')||'all';const mins=Number(prompt('Tự hết hạn sau bao nhiêu phút? (0 = không hết hạn)',existing.expiresAt?Math.max(0,Math.round((existing.expiresAt-Date.now())/60000)):0));const data={title:title.trim().slice(0,100),body:body.trim().slice(0,800),type,displayMode:mode,enabled:true,priority:Number(existing.priority)||0,updatedAtMs:Date.now(),startsAt:Date.now(),expiresAt:mins>0?Date.now()+mins*60000:null,createdBy:state.currentUser?.uid||''};try{if(!fbDb)throw new Error('Firebase unavailable');if(id)await fbDb.collection('announcements').doc(id).set(data,{merge:true});else await fbDb.collection('announcements').add(data);showToast('Announcement đã lưu.','success');await v51WriteAudit(id?'ANNOUNCEMENT_EDIT':'ANNOUNCEMENT_CREATE',{id,title});syncV51Announcements(true)}catch(error){showToast(`Không lưu announcement: ${error.message}`,'error')}}
  async function deleteAnnouncement(id){if(!fbDb||!confirm('Xóa announcement này?'))return;try{await fbDb.collection('announcements').doc(id).delete();await v51WriteAudit('ANNOUNCEMENT_DELETE',{id});showToast('Đã xóa.','success')}catch(error){showToast(`Không xóa được: ${error.message}`,'error')}}

  function syncV51Announcements(force=false){if(!fbDb)return;if(state.v51AnnouncementUnsub&&force){try{state.v51AnnouncementUnsub()}catch{}state.v51AnnouncementUnsub=null;state.v51AnnouncementsWired=false}if(state.v51AnnouncementsWired)return;state.v51AnnouncementsWired=true;try{state.v51AnnouncementUnsub=fbDb.collection('announcements').orderBy('updatedAtMs','desc').limit(50).onSnapshot(snap=>{state.announcementCache=snap.docs.map(d=>({id:d.id,...d.data()}));if(state.route==='menu')renderMenu();if(state.route==='admin'&&state.adminTab==='announcements')renderAdmin();renderHeader() },error=>{console.warn('announcement listener failed',error);state.v51AnnouncementsWired=false})}catch(error){state.v51AnnouncementsWired=false;console.warn('announcement listener failed',error)}}

  function securityAdminHtmlV51(){return `<div class="security-grid"><article class="security-card"><span class="eyebrow">CURRENT SESSION</span><h3>${escapeHtml(state.adminRole.toUpperCase())}</h3><p>UID: ${escapeHtml(state.currentUser?.uid||'—')}</p><p>Role source: Firebase ID token custom claim.</p><div class="security-kv"><span>Leaderboard lock</span><strong>${state.currentUser?.moderation?.leaderboardBlocked?'ON':'OFF'}</strong></div><div class="security-kv"><span>Event lock</span><strong>${state.currentUser?.moderation?.eventBlocked?'ON':'OFF'}</strong></div></article><article class="security-card"><span class="eyebrow">ADMIN / OWNER MODEL</span><h3>Client ≠ Authority</h3><p>DevTools có thể giả giao diện nhưng không tự tạo được Custom Claim. Role thật phải cấp bằng trusted server/Admin SDK.</p><div class="security-actions"><button class="secondary-btn" id="roleCommandBtn">🛡️ Manage Admin</button><button class="secondary-btn" id="registryBtn">👥 Role Directory</button></div></article><article class="security-card security-audit-card"><span class="eyebrow">AUDIT</span><h3>Recent role/security actions</h3><div id="securityAuditList" class="security-audit-list">Đang tải audit log...</div></article></div><div class="admin-action-note">Security Center chỉ hiển thị/điều phối quyền. Việc cấp hoặc thu hồi Custom Claim không được thực hiện bằng JavaScript trên browser; nút quản lý sẽ chuẩn bị lệnh trusted script và ghi dấu audit.</div>`;}
  async function loadSecurityAuditV51(){const host=$('securityAuditList');if(!host||!fbDb||!roleIsOwner())return;try{const snap=await fbDb.collection('auditLogs').orderBy('createdAt','desc').limit(12).get();host.innerHTML=snap.empty?'<div class="v51-notification-empty">Chưa có audit log.</div>':snap.docs.map(d=>{const x=d.data()||{};const when=x.createdAt?.toDate?x.createdAt.toDate().toLocaleString():'';return `<div class="security-audit-row"><strong>${escapeHtml(x.action||'ACTION')}</strong><span>${escapeHtml(x.actorRole||'')}</span><small>${escapeHtml(x.meta?.uid||x.meta?.eventId||'')} · ${escapeHtml(when)}</small></div>`}).join('')}catch(error){host.textContent='Không tải được audit log.';console.warn(error)}}

  function testLabHtmlV51(){return `<div class="test-lab-grid"><article class="test-lab-card"><span class="eyebrow">CHALLENGE GENERATOR</span><h3>Generate stress test</h3><p>Thử sinh nhiều board và kiểm tra seed/variant có bị trùng.</p><div class="test-lab-actions"><select id="testCount"><option value="10">10 boards</option><option value="50">50 boards</option><option value="100">100 boards</option></select><button class="primary-btn" id="generateTestBtn">🧪 Generate</button></div><pre id="testLabOutput">Ready.</pre></article><article class="test-lab-card"><span class="eyebrow">RANK EFFECT</span><h3>Preview power curve</h3><div class="test-rank-stack">${[...RANKS,...SPECIAL_RANKS].map(r=>`<button class="test-rank-preview ${r.id}" data-test-rank="${r.id}">${rankIconSvg(r,'mini')} <span>${escapeHtml(r.name)}</span><b>${r.power||0}</b></button>`).join('')}</div></article><article class="test-lab-card"><span class="eyebrow">REWARD / ACHIEVEMENT</span><h3>Inspect definitions</h3><p>${ACHIEVEMENT_DEFS.length} achievements · ${new Set(ACHIEVEMENT_DEFS.map(a=>a.title)).size} built-in titles.</p><button class="secondary-btn" id="testRewardBtn">🏆 Test Unlock Flow</button></article></div>`;}
  function runChallengeStressTest(){const count=Math.max(1,Math.min(100,Number($('testCount')?.value)||10));const seen=new Set(),duplicates=[];const all=allChallenges();for(let i=0;i<count;i++){const base=all[Math.floor(Math.random()*all.length)];if(!base)break;const v=createVariant(base);if(seen.has(v.seed))duplicates.push(v.seed);seen.add(v.seed)}const out=`Generated: ${seen.size}/${count}\nDuplicate seeds: ${duplicates.length}\nSource challenges: ${all.length}\nStatus: ${duplicates.length?'CHECK':'PASS'}`;$('testLabOutput').textContent=out;}

  function renderAdminV51(){
    if(!roleIsAdmin()){showToast('Bạn không có quyền Admin.','error');go('menu');return}
    const es=eventStore(); view.innerHTML=`<section class="view page-shell"><div class="page-head"><div class="page-title"><button class="nav-btn back-btn" id="adminBackBtn">← Menu</button><div><h2>${state.adminRole==='owner'?'Owner Control Center':'Admin Control Center'}</h2><div class="page-subtitle">Quản trị nội dung, Event, người chơi và bảo mật</div></div></div><div class="page-head-actions"><span class="admin-role-chip ${state.adminRole}">${escapeHtml(state.adminRole.toUpperCase())}</span></div></div><div class="page-content"><div class="admin-command-strip"><div><span class="eyebrow">CONTROL CENTER</span><strong>V5.1 ADMIN & OWNER EXPANSION</strong><small>Rank cosmetic độc quyền không tác động clear/leaderboard.</small></div><div class="admin-strip-actions">${roleIsOwner()?'<button class="secondary-btn" id="grantAdminBtn">🛡️ Manage Admin</button>':''}<button class="secondary-btn" id="adminRefreshBtn">↻ Refresh</button></div></div>${v51AdminNav()}<div class="admin-tab-content">${state.adminTab==='dashboard'?adminDashboardHtmlV51(es):state.adminTab==='players'?playersAdminHtmlV51():state.adminTab==='events'?eventsAdminHtmlV51(es):state.adminTab==='announcements'?announcementsAdminHtmlV51():state.adminTab==='security'?securityAdminHtmlV51():testLabHtmlV51()}</div></div></section>`;
    $('adminBackBtn').onclick=()=>go('menu'); $('adminRefreshBtn').onclick=renderAdmin; $('grantAdminBtn')?.addEventListener('click',ownerGrantOrRevokeAdminFlow); document.querySelectorAll('.admin-tab').forEach(b=>b.onclick=()=>{state.adminTab=b.dataset.adminTab;v5Write(V51.storage.adminTab,state.adminTab);renderAdmin()});
    document.querySelectorAll('.view-admin-player').forEach(b=>b.onclick=()=>openProfile(b.dataset.player)); document.querySelectorAll('.grant-title').forEach(b=>b.onclick=()=>ownerModifyPlayer(b.dataset.player,'title')); document.querySelectorAll('.lb-block-btn').forEach(b=>b.onclick=()=>ownerModifyPlayer(b.dataset.player,'leaderboardBlock')); document.querySelectorAll('.event-block-btn').forEach(b=>b.onclick=()=>ownerModifyPlayer(b.dataset.player,'eventBlock')); document.querySelectorAll('.moderation-btn').forEach(b=>b.onclick=()=>ownerModifyPlayer(b.dataset.player,'moderation')); document.querySelectorAll('.reset-stats-btn').forEach(b=>b.onclick=()=>ownerModifyPlayer(b.dataset.player,'reset')); document.querySelectorAll('.role-btn').forEach(b=>b.onclick=()=>ownerGrantOrRevokeAdminFlowForUid(b.dataset.player));
    $('adminPlayerSearch')?.addEventListener('input',e=>{const q=e.target.value.toLowerCase();document.querySelectorAll('[data-player-row]').forEach(row=>row.style.display=row.textContent.toLowerCase().includes(q)?'':'none')});
    $('newEventBtn')?.addEventListener('click',()=>adminCreateEvent()); $('refreshAdminBtn')?.addEventListener('click',renderAdmin); document.querySelectorAll('.edit-event').forEach(b=>b.onclick=()=>adminEditEvent(b)); document.querySelectorAll('.publish-event').forEach(b=>b.onclick=()=>adminTransition(b,'PUBLISHED')); document.querySelectorAll('.start-live-event').forEach(b=>b.onclick=()=>adminTransition(b,'LIVE')); document.querySelectorAll('.close-event').forEach(b=>b.onclick=()=>adminTransition(b,'CLOSED')); document.querySelectorAll('.announce-event').forEach(b=>b.onclick=()=>adminTransition(b,'ANNOUNCED')); document.querySelectorAll('.archive-event').forEach(b=>b.onclick=()=>adminTransition(b,'ARCHIVED')); document.querySelectorAll('.duplicate-event').forEach(b=>b.onclick=()=>adminDuplicate(b)); document.querySelectorAll('.monitor-event').forEach(b=>b.onclick=()=>{const id=b.closest('.admin-event-card').querySelector('.admin-event-meta').dataset.event;state.adminTab='events';go('events');setTimeout(()=>showEventBoard(id),40)});
    $('newAnnouncementBtn')?.addEventListener('click',()=>createOrEditAnnouncement()); document.querySelectorAll('.edit-announcement').forEach(b=>b.onclick=()=>createOrEditAnnouncement(b.dataset.ann)); document.querySelectorAll('.delete-announcement').forEach(b=>b.onclick=()=>deleteAnnouncement(b.dataset.ann)); $('roleCommandBtn')?.addEventListener('click',ownerGrantOrRevokeAdminFlow); $('registryBtn')?.addEventListener('click',async()=>{if(!roleIsOwner())return showToast('Chỉ Owner mới xem được Role Directory.','error');try{const s=await fbDb.collection('adminRegistry').limit(100).get();const rows=s.docs.map(d=>d.data()).map(x=>`${x.role?.toUpperCase()||'ROLE'} · ${x.uid}`).join('\n')||'Chưa có registry.';alert(`ROLE DIRECTORY\n\n${rows}`)}catch(error){showToast(`Không tải registry: ${error.message}`,'error')}}); if(state.adminTab==='security')loadSecurityAuditV51(); $('generateTestBtn')?.addEventListener('click',runChallengeStressTest); $('testRewardBtn')?.addEventListener('click',()=>showToast('Reward flow test: dùng definitions hiện tại, không ghi leaderboard.','success')); document.querySelectorAll('.test-rank-preview').forEach(b=>b.onclick=()=>openRankReview(b.dataset.testRank));
  }
  renderAdmin=renderAdminV51;

  function v51UserSpecialSync(){if(!state.currentUser)return;const special=specialRankForRole(state.adminRole);if(!special)return;const ach=state.adminRole==='owner'? 'theboss-role':'addmin-role';const title=state.adminRole==='owner'?V51.ownerTitle:V51.adminTitle;const p=v5Progress();if(!p.unlocked.includes(ach)){p.unlocked.push(ach);v5SaveProgress(p)}if(!v5Titles().some(t=>(t.name||t)===title))unlockTitle(title,state.adminRole==='owner'?V51.ownerAchievement:V51.adminAchievement,'special');const existing=state.currentUser.equippedRank||localStorage.getItem(`${V51.storage.equippedRank}_${state.currentUser.uid}`);if(!existing){const specialId=state.adminRole==='owner'?'cane':'admin-special';state.currentUser.equippedRank=specialId;localStorage.setItem(`${V51.storage.equippedRank}_${state.currentUser.uid}`,specialId);try{fbDb?.collection('users').doc(state.currentUser.uid).update({equippedRank:specialId,roleLabel:state.adminRole,updatedAt:firebase.firestore.FieldValue.serverTimestamp()})}catch{}}else{state.currentUser.roleLabel=state.adminRole;try{fbDb?.collection('users').doc(state.currentUser.uid).update({roleLabel:state.adminRole,updatedAt:firebase.firestore.FieldValue.serverTimestamp()})}catch{}}}

  const oldInitFirebaseV51 = initFirebase;
  initFirebase = function(){oldInitFirebaseV51();if(fbDb){syncV51Announcements();fbAuth?.onAuthStateChanged(async u=>{if(!u)return;try{const tok=await u.getIdTokenResult();state.adminRole=tok.claims.role||'user';const snap=await fbDb.collection('users').doc(u.uid).get();if(snap.exists){const shaped=ensureUserShape({key:snap.id,...snap.data(),roleLabel:state.adminRole});state.currentUser={...state.currentUser,...shaped,key:u.uid,uid:u.uid,name:shaped.displayName};}v51UserSpecialSync();renderHeader();if(['menu','rank','achievements','admin'].includes(state.route))renderRoute();}catch(error){console.warn('V5.1 role/profile sync failed',error)}})}};

  const oldRenderRouteV51 = renderRoute;
  renderRoute = function(){stopRankAudio();document.getElementById('rankReviewRoot')?.remove();if(state.route==='speedrun'&&state.running)stopRun(false);state.route=routeFromHash();state.currentUser=currentUser();renderHeader();if(state.route!=='speedrun'&&state.editor){state.editor.dispose();state.editor=null;state.monacoReady=false;state.challenge=null}if(state.route==='menu')renderMenu();else if(state.route==='speedrun')renderSpeedrun();else if(state.route==='rank')renderRank();else if(state.route==='leaderboard')renderLeaderboard();else if(state.route==='academy')renderAcademy();else if(state.route==='achievements')renderAchievements();else if(state.route==='events')renderEvents();else if(state.route==='admin')renderAdmin();if(window.lucide)window.lucide.createIcons();wireAnnouncementBar()};
  syncV51Announcements();
